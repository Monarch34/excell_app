...
import ChartAnnotationDialog from '@/features/charts/ChartAnnotationDialog.vue';
...
const ChartDefinitionForm = defineAsyncComponent({
  loader: () => import('@/features/charts/ChartDefinitionForm.vue'),
  errorComponent: AsyncErrorFallback,
  timeout: 10000,
});
const ChartRenderer = defineAsyncComponent({
  loader: () => import('@/features/charts/ChartRenderer.vue'),
  errorComponent: AsyncErrorFallback,
  timeout: 10000,
});
...
const chartColumnsExist = computed(() =>
  chartDataReady(chart.value, chartData.value as Record<string, unknown>[]),
);
