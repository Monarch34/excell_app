...
import type { AnalysisConfig, ChartSpec } from '@/types/domain';
...
function getColorText(color: string, isDefault = false): string {
  ...
  const normalized = normalizeColorToHex(resolved);
  return isDefault ? `${normalized} (default)` : normalized;
}

function normalizeColorToHex(color: string | null | undefined): string {
  if (!color) return 'Not defined';
  ...
  if (/^rgba?\(/i.test(value)) {
    ...
    if (!channels || channels.length < 3) return value.toUpperCase();
