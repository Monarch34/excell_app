...
import type { FormulaPreviewErrorCode } from '@/types/api';
import CalculationItemsPanel from '@/components/steps/calculations/CalculationItemsPanel.vue';
import CalculationEditorPanel from '@/components/steps/calculations/CalculationEditorPanel.vue';
...
async function handlePreview(formulaOverride?: string) {
  ...
  try {
    ...
    const user_formulas = derivedColumns.value
    ...
    const derived_parameters = derivedColumns.value
      .filter(d => d.id !== activeColumn.value?.id && d.enabled && d.formula && d.type === 'parameter')
    ...
    <div class="grid" :class="{ 'ui-calc-grid-center-left': isLeftEmpty }">
      ...
      <div class="col-12 lg:col-4 ui-calc-left-panel">
        <CalculationItemsPanel
          ...
          @select="(id) => derivedStore.setActiveColumn(id)"
          @toggle="(id) => derivedStore.toggleEnabled(id)"
