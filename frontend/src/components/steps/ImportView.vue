...
import ImportUploadPanel from '@/components/steps/import/ImportUploadPanel.vue';
import ImportDataPreviewPanel from '@/components/steps/import/ImportDataPreviewPanel.vue';
...
const isDragging = upload.isDragging;
const showConfig = upload.showConfig;
const configFile = upload.configFile;
const fileSummary = upload.fileSummary;
...
async function onFileSelect(event: Event): Promise<void> {
  ...
  await upload.handleUpload(file);
  target.value = '';
}
...
<template>
  <div class="step-shell">
    <ImportUploadPanel
      ...
      @drop="onDrop"
      @configFileSelect="upload.onConfigFileSelect"
      @clearConfigFile="upload.clearConfigFile"
