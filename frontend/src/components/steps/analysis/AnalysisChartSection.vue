...
const chartColumnsExist = computed(() => {
  if (!props.chart.chartType) return false;
  if (!props.chart.xColumn || props.chart.yColumns.length === 0) return false;
  if (props.data.length === 0) return false;

  const firstRow = props.data[0];
  const xExists = props.chart.xColumn in firstRow;
  const yExists = props.chart.yColumns.every((column) => column in firstRow);
